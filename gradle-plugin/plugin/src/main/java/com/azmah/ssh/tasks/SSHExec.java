...
public abstract class SSHExec extends AbstractRemoteTask {
    ...
    public void runCommand() throws GradleException {
        getRemotes().get().forEach(remote -> {
            try (ClientSession session = getSession(remote.getHost().get(), remote.getPort().get(), remote.getUser().get(), remote.getPassword().get())) {
                try (OutputStream stdout = new ByteArrayOutputStream();
                     OutputStream stderr = new ByteArrayOutputStream();
                     ClientChannel channel = session.createExecChannel(getCommand().get())) {
